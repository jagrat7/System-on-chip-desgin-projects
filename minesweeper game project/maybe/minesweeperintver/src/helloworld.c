...
void BTN_Intr_Handler(void *InstancePtr)
{
	...
	if(btn_value>0)
	{
		...
		else if(gameon==0)
		{
			...
			xil_printf("Press button 3 and switch 3 to restart\n\r");
		}
		else
		{
		...
		if(gameon==0 && btn_value==3 && sw_data==4)
		{
			fristpress=0;
			gameon=1;
			userboard(0,0);
...
void userboard(int x,int y)
{
	...
	else
	{
		...
		{
			//score=score+realbr[x][y];

			//inserting the inputted values in the  user field
			if ( (realbr[x-1][ y] != 8) && (realbr[x+1][ y] != 8 ) && (realbr[x][ y+1] != 8) && (realbr[x][ y-1] != 8)  )
			{
				userbr[x][y]=realbr[x][y]+'0';
				userbr[x-1][y]=realbr[x-1][ y]+'0';
				//score=score+realbr[x-1][y];
				userbr[x+1][y]=realbr[x+1][ y]+'0';
				//score=score+realbr[x+1][y];
				userbr[x][y+1]=realbr[x][ y+1]+'0';
				//score=score+realbr[x][ y+1];
				userbr[x][y-1]=realbr[x][ y-1]+'0';
				//score=score+realbr[x][ y-1];
...
//	else
//	{
		xil_printf("Enter your location to explore; use the switches to select a row and press a button to confirm your location (its on row 0 without any switches pressed)\n\r");
	...
	//}
		if(gameon==0)
		{
			xil_printf("BOOM! Game Over!\n\r");
			xil_printf("Press button 3 and switch 3 to restart\r");
		}
		xtemp=x;
		ytemp=y;
}
